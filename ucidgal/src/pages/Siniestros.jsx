...
const EMPTY_FORM = {
  ...
  lugar_fallecimiento: '', tipo_servicio: 'repatriacion',
  pais_destino: '', cert_defuncion: 'pendiente',
  permiso_traslado: 'pendiente', observaciones: '', estado: 'abierto'
}
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: s }, { data: a }] = await Promise.all([
      supabase.from('siniestros')
        .select('*, afiliados(nombre, apellidos, num_inscripcion)')
        ...
        {loading
          ...
          : <div className="table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>Expediente</th><th>Fallecido</th><th>Titular</th>
                    <th>Tipo</th><th>Fecha</th><th>Destino</th><th>Estado</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.length === 0
                    ...
                    : filtered.map(s => (
                      <tr key={s.id}>
                        ...
                        <td>{labels[s.tipo_servicio] || s.tipo_servicio}</td>
                        <td style={{color:'var(--muted)'}}>{s.fecha_fallecimiento || '—'}</td>
                        <td>{s.pais_destino || s.lugar_fallecimiento || '—'}</td>
                        <td><span className={`badge badge-${s.estado}`}>{labels[s.estado] || s.estado}</span></td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div className="form-section" style={{marginBottom:'1rem'}}>Estado de documentación</div>
              <div className="form-grid" style={{marginBottom:'1rem'}}>
                <div className="form-group">
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              {[
                ...
                ['Lugar de fallecimiento', selected.lugar_fallecimiento || '—'],
                ['Destino', selected.pais_destino || '—'],
                ['Certificado de defunción', <span className={`badge badge-${selected.cert_defuncion === 'recibido' ? 'activo' : 'pendiente'}`}>{labels[selected.cert_defuncion]}</span>],
                ['Permiso de traslado', <span className={`badge badge-${selected.permiso_traslado === 'obtenido' ? 'activo' : 'pendiente'}`}>{labels[selected.permiso_traslado]}</span>],
                ['Estado expediente', <span className={`badge badge-${selected.estado}`}>{labels[selected.estado]}</span>],
              ...
              <div style={{marginTop:'1.25rem'}}>
                <div className="form-section" style={{marginBottom:'0.75rem'}}>Actualizar estado</div>
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                  {['abierto','procesando','cerrado'].map(e => (
                    <button key={e} className={`btn btn-sm ${selected.estado === e ? 'btn-primary' : ''}`}
                      onClick={() => { actualizarEstado(selected.id, 'estado', e); setSelected(s => ({...s, estado: e})) }}>
                      {labels[e]}
                    </button>
                  ))}
                </div>
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap',marginTop:'8px'}}>
                  <button className={`btn btn-sm ${selected.cert_defuncion === 'recibido' ? 'btn-primary' : ''}`}
                    onClick={() => { actualizarEstado(selected.id, 'cert_defuncion', 'recibido'); setSelected(s => ({...s, cert_defuncion:'recibido'})) }}>
                    Cert. defunción recibido
                  </button>
                  <button className={`btn btn-sm ${selected.permiso_traslado === 'obtenido' ? 'btn-primary' : ''}`}
                    onClick={() => { actualizarEstado(selected.id, 'permiso_traslado', 'obtenido'); setSelected(s => ({...s, permiso_traslado:'obtenido'})) }}>
                    Permiso de traslado obtenido
                  </button>
                </div>
              </div>
            </div>
