...
export default function BuscadorGlobal() {
  ...
  const buscar = useCallback(async (q) => {
    ...
    setLoading(true)
    const like = `%${q}%`
    const [{ data: afs }, { data: sins }, { data: cuotas }] = await Promise.all([
    ...
    const cuotasFiltradas = (cuotas || []).filter(c =>
      `${c.afiliados?.nombre} ${c.afiliados?.apellidos} ${c.afiliados?.num_inscripcion}`.toLowerCase().includes(q.toLowerCase())
    ).slice(0, 4)
