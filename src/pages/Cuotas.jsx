  ...
  async function guardar() {
    ...
    if (error) {
      ...
      if (form._justFile && cuotaGuardada?.id) {
        ...
        if (path) await supabase.from('cuotas').update({ justificante_url: path }).eq('id', cuotaGuardada.id)
      }
      ...
      if (payload.estado === 'pagado' && payload.fecha_pago) {
        ...
        // Also reactivate baja_impago afiliados when they pay
                  ...
                  {filtered.length === 0
                    ...
                    : filtered.map(c => (
                      <tr key={c.id}>
                        ...
                        <td>{c.justificante_url || c.referencia
                          ? c.justificante_url?.startsWith('justificantes/')
                            ? <button className="btn btn-sm" onClick={async () => {
                                const { data } = await supabase.storage.from('documentos-afiliados').createSignedUrl(c.justificante_url || c.referencia, 60)
