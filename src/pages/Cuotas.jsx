...
export default function Cuotas() {
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: c }, { data: a }] = await Promise.all([
      ...
      supabase.from('afiliados').select('id, nombre, apellidos, num_inscripcion, tipo_inscripcion, padres_convivientes, estado').not('estado', 'in', '("baja_voluntaria","baja")').order('num_inscripcion')
    ])
    setCuotas(c || [])
    setAfiliados(a || [])
    setLoading(false)
  ...
  function exportarExcel() {
    ...
    script.onload = () => {
      ...
      const datos = filtered.map(c => {
        let validoHasta = '—'
        if (c.fecha_pago && c.estado !== 'anulado') {
          const fin = new Date(c.fecha_pago)
          fin.setDate(fin.getDate() + 90)
          fin.setFullYear(fin.getFullYear() + 1)
          validoHasta = fin.toLocaleDateString('es-ES')
        }
        return {
      ...
      ws['!cols'] = [{wch:14},{wch:24},{wch:6},{wch:12},{wch:14},{wch:14},{wch:20},{wch:12}]
  ...
  function exportarPDF() {
    ...
    const filas = filtered.map(c => {
      ...
      <tr>
        ...
        <td>${validoHasta}</td>
        <td>${c.referencia || '—'}</td>
        <td><span class="${c.estado}">${c.estado === 'anulado' ? 'Anulado' : 'Pagado'}</span></td>
      </tr>`
    }).join('')
    ...
    w.document.write(`
      ...
      <style>
        ...
        .anulado { color: #A32D2D; font-weight:500; text-decoration: line-through; }
  ...
  async function subirJustificante(file, cuotaId) {
    ...
    if (!allowed.includes(ext)) {
      setAlerta({ tipo:'error', titulo:'Formato no permitido', mensaje:'Solo se permiten archivos PDF, JPG, PNG o WEBP.' }); return null
    }
    ...
    if (error) { setAlerta({ tipo:'error', titulo:'Error al subir', mensaje:'No se pudo subir el justificante: ' + error.message }); return null }
    return path
  }
  ...
  async function guardar() {
    ...
    if (error) {
      ...
      if (payload.estado === 'pagado' && payload.fecha_pago) {
        // Check if this is a renewal (has previous payments) or new member
        const { data: cuotasPrevias } = await supabase
          .from('cuotas')
          .select('id')
          .eq('afiliado_id', payload.afiliado_id)
          .eq('estado', 'pagado')
          .not('id', 'eq', cuotaGuardada.id)
          .limit(1)

        const esRenovacion = cuotasPrevias && cuotasPrevias.length > 0

        const fechaPago = new Date(payload.fecha_pago)
        const hoy = new Date()
        const diasDesde = Math.floor((hoy - fechaPago) / 86400000)

        // Renovations stay active, new members get carencia
        const nuevoEstado = esRenovacion ? 'activo' : (diasDesde < 90 ? 'en_carencia' : 'activo')

        await supabase.from('afiliados')
          .update({ estado: nuevoEstado, updated_at: new Date().toISOString() })
          .eq('id', payload.afiliado_id)
          .not('estado', 'in', '(baja_voluntaria,baja)')
      }
      await fetch()
      closeModal()
    }
  ...
  // IDs que ya pagaron en el año seleccionado en el formulario
  const anioSeleccionado = Number(form.anio) || ANIO_ACTUAL
  const idsPagadosEnAnio = new Set(
    cuotas
      .filter(c => Number(c.anio) === anioSeleccionado && c.estado !== 'anulado')
      .map(c => c.afiliado_id)
  )
  const afiliadosDisponibles = modal === 'editar'
    ? afiliados
    : afiliados.filter(a => !idsPagadosEnAnio.has(a.id))
        ...
        {loading
          ...
          : <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {filtered.length === 0
                    ? <tr><td colSpan={9} style={{textAlign:'center',color:'var(--muted)',padding:'2rem'}}>No hay registros para los filtros seleccionados.</td></tr>
                    : filtered.map(c => (
                      <tr key={c.id}>
                          ...
                          {c.estado !== 'anulado'
                            ? <button className="btn btn-sm" style={{color:'#854F0B',borderColor:'rgba(133,79,11,0.25)'}}
                                onClick={() => setConfirmar({
                                  titulo: 'Anular pago',
                                  mensaje: `¿Confirmas que deseas anular este pago? El registro se conservará pero no contará en los totales de recaudación.`,
                                  labelConfirmar: 'Anular pago',
                                  tipo: 'warn',
                                  accion: async () => { await supabase.from('cuotas').update({ estado: 'anulado' }).eq('id', c.id); fetch() }
                                })}>Anular</button>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              {form.afiliado_id && afiliados.find(a => a.id === form.afiliado_id)?.estado === 'baja_impago' && (
                <div className="alert alert-warn" style={{marginBottom:'1rem'}}>
                  <strong>⚠ Afiliado en baja por impago.</strong> Al registrar este pago el afiliado quedará reactivado automáticamente con estado "Pendiente pago" y comenzará un nuevo periodo de carencia de 90 días.
                </div>
              )}
              <div className="form-grid" style={{marginBottom:'1rem'}}>
                    ...
                    : <>
                        ...
                        {afiliadosDisponibles.length === 0
                          ? <div style={{padding:'8px 10px',background:'var(--bg)',borderRadius:'var(--radius-sm)',fontSize:'12px',color:'var(--muted)'}}>
                              Todos los afiliados ya tienen pago registrado para {anioSeleccionado}.
                            </div>
                          : (() => {
                ...
                <div className="form-group">
                  ...
                  <select className="form-select" value={form.anio} onChange={e => { updateForm('anio', e.target.value); updateForm('afiliado_id', ''); setBusqAfiliado('') }}>
