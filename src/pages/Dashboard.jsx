...
export default function Dashboard() {
  const [stats, setStats]             = useState({ afiliados: 0, activos: 0, cuotas: 0, siniestros: 0, sinistrosTotal: 0, gastos: 0 })
  ...
  const fetchData = useCallback(async () => {
    ...
    const totalEstados = Object.values(emap).reduce((a, b) => a + b, 0) || 1
    setEstados(estadosOrden
      .filter(e => emap[e] > 0)
      .map(e => ({ e, n: emap[e], pct: Math.round((emap[e] / totalEstados) * 100) }))
    )
    ...
    const totalProv = sorted.reduce((sum, [, n]) => sum + n, 0) || 1
    setProvincias(sorted.map(([p, n]) => ({ p, n, pct: Math.round((n / totalProv) * 100) })))
