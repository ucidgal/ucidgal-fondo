...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    const [
      ...
      supabase.from('siniestros').select('gasto_total').not('gasto_total', 'is', null),
    ...
    const totalGastos = (gastosRes.data || []).reduce((s, s2) => s + Number(s2.gasto_total || 0), 0)
  ...
  return (
    <div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-label">Gastos totales expedientes</div>
          <div className="stat-value">{stats.gastos.toLocaleString('es-ES')}€</div>
          <div className="stat-sub">Todos los años</div>
        </div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1rem'}}>
        ...
        <div className="card">
          <div className="card-header">
            ...
            {impagados.length > 0 && (
              <button className="btn btn-sm btn-danger" onClick={darBajaMasiva} disabled={bajando}>
                {bajando ? 'Procesando...' : `Baja por impago (${impagados.length})`}
              </button>
            )}
          </div>
