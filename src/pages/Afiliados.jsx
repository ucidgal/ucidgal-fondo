...
const ESTADOS_CONFIG = {
  activo:           { label: 'Activo',            bg: '#E8F8FC', color: '#1A8FA6', badge: 'badge-activo' },
  en_carencia:      { label: 'En carencia',        bg: '#E6F1FB', color: '#185FA5', badge: 'badge-procesando' },
  pendiente_pago:   { label: 'Pendiente pago',     bg: '#FAEEDA', color: '#633806', badge: 'badge-pendiente' },
  impagado:         { label: 'Baja por impago',    bg: '#FCEBEB', color: '#791F1F', badge: 'badge-baja' },
  baja_voluntaria:  { label: 'Baja voluntaria',    bg: '#F1F0EC', color: '#5F5E5A', badge: 'badge-baja' },
  baja_impago:      { label: 'Baja por impago',    bg: '#FCEBEB', color: '#791F1F', badge: 'badge-baja' },
  baja:             { label: 'Baja',               bg: '#F1F0EC', color: '#5F5E5A', badge: 'badge-baja' },
  pendiente:        { label: 'Pendiente',          bg: '#FAEEDA', color: '#633806', badge: 'badge-pendiente' },
}

function EstadoBadge({ estado, estadoCalculado }) {
  const est = estadoCalculado || estado
  const cfg = ESTADOS_CONFIG[est] || ESTADOS_CONFIG[estado] || { label: est, bg: '#F1F0EC', color: '#5F5E5A' }
  return (
    <span style={{
...
const PAISES = [
  'Afganistán','Albania','Alemania','Algeria','Angola','Arabia Saudita','Argelia','Argentina',
  'Armenia','Australia','Austria','Azerbaiyán','Bahrein','Bangladesh','Bélgica','Benín',
  'Bolivia','Bosnia y Herzegovina','Brasil','Bulgaria','Burkina Faso','Burundi','Camerún',
  'Canadá','Chad','Chile','China','Colombia','Comoras','Congo','Costa de Marfil','Croacia',
  'Dinamarca','Djibouti','Ecuador','Egipto','Emiratos Árabes Unidos','Eritrea','Eslovakia',
  'Eslovenia','España','Estonia','Etiopía','Filipinas','Finlandia','Francia','Gabón','Gambia',
  'Georgia','Ghana','Grecia','Guatemala','Guinea','Guinea-Bisáu','Guinea Ecuatorial','Haití',
  'Honduras','Hungría','India','Indonesia','Irak','Irán','Irlanda','Islas Comoras','Israel',
  'Italia','Jamaica','Japón','Jordania','Kazajistán','Kenia','Kosovo','Kuwait','Kirguistán',
  'Líbano','Liberia','Libia','Lituania','Luxemburgo','Macedonia del Norte','Madagascar','Malasia',
  'Maldivas','Malí','Malta','Marruecos','Mauritania','México','Moldova','Mongolia','Montenegro',
  'Mozambique','Myanmar','Nepal','Nicaragua','Níger','Nigeria','Noruega','Nueva Zelanda','Omán',
  'Países Bajos','Pakistán','Palestina','Panamá','Paraguay','Perú','Polonia','Portugal',
  'Qatar','Reino Unido','República Centroafricana','República Checa','República Dominicana',
  'Rumanía','Rusia','Rwanda','Senegal','Serbia','Sierra Leona','Siria','Somalia','Sri Lanka',
  'Sudáfrica','Sudán','Sudán del Sur','Suecia','Suiza','Tailandia','Tanzania','Tayikistán',
  'Togo','Túnez','Turquía','Turkmenistán','Uganda','Ucrania','Uruguay','Uzbekistán',
  'Venezuela','Yemen','Yibuti','Zambia','Zimbabwe',
]
...
const EMPTY_FORM = {
  ...
  estado: 'pendiente_pago', matrimonio_mixto: false, hijos: [], aceptaReglamento: false
}
    ...
    w.document.write(`
      ...
      <body><div class="card">
        ...
        <div class="body">
          ...
          ${(afiliado.hijos && afiliado.hijos.length > 0) ? `
            <div style="margin-top:10px;padding-top:10px;border-top:1px solid #f0f0f0;">
              <div style="font-size:11px;font-weight:600;color:#6b6b68;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">Beneficiarios</div>
              ${afiliado.hijos.map(h => `
                <div class="row" style="padding:3px 0;">
                  <span class="lbl" style="font-size:11px;">${h.tipo === 'conyuge' ? 'Cónyuge' : h.tipo === 'padre' ? 'Padre/Madre' : 'Hijo/a'}</span>
                  <span class="val" style="font-size:12px;">${h.nombre} ${h.apellidos || ''}</span>
                </div>`).join('')}
            </div>` : ''}
        </div>
        ...
        <div className="modal-body">
          ...
          {afiliado.hijos && afiliado.hijos.length > 0 && (
            <div style={{marginTop:'0.75rem',paddingTop:'0.75rem',borderTop:'1px solid var(--border)'}}>
              <div style={{fontSize:'11px',fontWeight:500,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:'6px'}}>Beneficiarios</div>
              {afiliado.hijos.map((h, i) => (
                <div key={i} style={{display:'flex',alignItems:'center',gap:'8px',padding:'5px 0',borderBottom:'0.5px solid var(--border)'}}>
                  <span style={{fontSize:'10px',fontWeight:500,padding:'1px 7px',borderRadius:'20px',flexShrink:0,
                  ...
                  <span style={{fontSize:'13px',fontWeight:500}}>{h.nombre} {h.apellidos || ''}</span>
                  {h.fecha_nacimiento && (
                    <span style={{fontSize:'11px',color:'var(--muted)',marginLeft:'auto'}}>
                      {Math.floor((new Date()-new Date(h.fecha_nacimiento))/(365.25*86400000))} años
                    </span>
                  )}
                </div>
              ))}
...
export default function Afiliados() {
  ...
  const [filterProvincia, setFilterProv] = useState('todos')
  ...
  const [paisFijado, setPaisFijado]   = useState(false)
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('afiliados_estado_real').select('*').order('num_inscripcion')
    setAfiliados(data || [])
    setLoading(false)
  }, [])
  ...
  const filtrosActivos = [filterEstado, filterTipo, filterProvincia].filter(f => f !== 'todos').length

  useEffect(() => {
    let r = afiliados
    if (search)               r = r.filter(a => `${a.nombre} ${a.apellidos} ${a.dni_nie}`.toLowerCase().includes(search.toLowerCase()))
    if (filterEstado !== 'todos') r = r.filter(a => (a.estado_calculado || a.estado) === filterEstado)
    if (filterTipo       !== 'todos') r = r.filter(a => a.tipo_inscripcion === filterTipo)
    if (filterProvincia !== 'todos') r = r.filter(a => a.provincia === filterProvincia)
    setFiltered(r)
  }, [afiliados, search, filterEstado, filterTipo, filterProvincia])

  function limpiarFiltros() {
    setFilter('todos'); setFilterTipo('todos'); setFilterProv('todos'); setSearch('')
  }

  function openNuevo()   { setForm(EMPTY_FORM); setModal('nuevo'); setMsg(null); setPaso(1); setErrores({}); setPaisFijado(false) }
  function openEditar(a) { setSelected(a); setForm({ ...a }); setModal('editar'); setMsg(null); setPaso(1); setErrores({}); setPaisFijado(!!a.pais_origen) }
  function openDetalle(a){ setSelected(a); setModal('detalle'); setTabDetalle('datos'); setHistorial([]); setDocumentos([]) }
  ...
  async function fetchDocumentos(id) {
    setLoadingDocs(true)
    const { data } = await supabase.storage.from('documentos-afiliados').list(id, { sortBy: { column: 'created_at', order: 'desc' } })
    setDocumentos(data || [])
  ...
  async function subirDocumento(tipo, file) {
    ...
    if (!allowed.includes(ext)) {
      setAlerta({ tipo:'error', titulo:'Formato no permitido', mensaje:'Solo se permiten archivos PDF, JPG o PNG.' })
      return
    }
    if (file.size > 20 * 1024 * 1024) {
      setAlerta({ tipo:'warn', titulo:'Archivo demasiado grande', mensaje:'El archivo no puede superar 20 MB. Reduce el tamaño e inténtalo de nuevo.' })
      return
    }
    setSubiendo(tipo)
    ...
    const path = `${selected.id}/${tipo}_${Date.now()}.${finalExt}`
    const { error } = await supabase.storage.from('documentos-afiliados').upload(path, fileToUpload, { upsert: false })
    if (error) { setAlerta({ tipo:'error', titulo:'Error al subir', mensaje:'No se pudo subir el archivo: ' + error.message }); return }
    else await fetchDocumentos(selected.id)
    setSubiendo(null)
  }

  async function eliminarDocumento(path) {
    await supabase.storage.from('documentos-afiliados').remove([path])
    await fetchDocumentos(selected.id)
  }

  async function verDocumento(path) {
    const { data } = await supabase.storage.from('documentos-afiliados').createSignedUrl(path, 60)
  ...
  function validarPaso(p) {
    ...
    if (p === 3) {
      ...
      // Art. 16 — reglamento acceptance required
      if (modal === 'nuevo' && !form.aceptaReglamento) {
        e.aceptaReglamento = 'Debes aceptar el reglamento del fondo para completar la inscripción (Art. 16).'
      }
      // Validate padres count matches registered padre members
      const padresIndicados  = Number(form.padres_convivientes) || 0
      const padresRegistrados = (form.hijos || []).filter(h => h.tipo === 'padre').length
      if (padresIndicados > 0 && padresRegistrados < padresIndicados) {
        e.padres_faltantes = `Debes añadir los datos de ${padresIndicados - padresRegistrados} padre(s)/madre(s) conviviente(s) que faltan. Usa el botón "+ Padre/Madre" en la sección de miembros.`
      }
      // Validate familia has conyuge only if Casado/a
      if (form.tipo_inscripcion === 'familia' && form.estado_civil === 'Casado/a') {
        const conyuges = (form.hijos || []).filter(h => h.tipo === 'conyuge').length
        if (conyuges === 0) e.conyuge_faltante = 'En inscripción familiar con estado civil Casado/a debes añadir los datos del cónyuge. Usa el botón "+ Cónyuge".'
      }
    }
    if (p === 4) {
      if (modal === 'nuevo') {
        if (!form._doc_dni) {
          e._doc_dni = 'El documento DNI/NIE/Pasaporte es obligatorio'
        }
        if (!form._doc_empadronamiento) {
          e._doc_empadronamiento = 'El empadronamiento colectivo es obligatorio'
        }
        if (form.matrimonio_mixto && !form._doc_acta_notarial) {
          e._doc_acta_notarial = 'El acta notarial es obligatoria para matrimonio mixto (Art. 15)'
        }
      }
    }
    return e
  ...
  async function guardar() {
    ...
    if (form.dni_nie && form.dni_nie.trim()) {
      let query = supabase.from('afiliados').select('id, nombre, apellidos, num_inscripcion').eq('dni_nie', form.dni_nie.trim().toUpperCase())
    ...
    const { data: { session } } = await supabase.auth.getSession()
    const email = session?.user?.email || 'admin'
    // Exclude fields that don't exist in the DB
    const { aceptaReglamento: _, _justFile: __, ...formData } = form
    const payload = {
      ...formData,
      dni_nie: form.dni_nie?.trim().toUpperCase(),
      num_miembros: 1 + (form.hijos?.filter(h => h.nombre?.trim()).length || 0),
      padres_convivientes: (form.hijos || []).filter(h => h.tipo === 'padre' && h.nombre?.trim()).length,
    ...
    if (error) {
      ...
      if (modal === 'nuevo') {
        await registrarHistorial(result.id, 'creacion', [{ campo: 'registro', anterior: null, nuevo: form.aceptaReglamento ? 'Alta en el sistema · Reglamento aceptado (Art. 16)' : 'Alta en el sistema' }], email)
      // Upload documents from paso 4
      const docTypes = ['dni', 'empadronamiento', 'acta_notarial']
      for (const tipo of docTypes) {
        const file = form[`_doc_${tipo}`]
        if (!file) continue
        const ext = file.name.split('.').pop().toLowerCase()
        const fileToUp = ['jpg','jpeg','png','webp'].includes(ext) ? await comprimirImagen(file) : file
        const finalExt = ['jpg','jpeg','png','webp'].includes(ext) ? 'jpg' : ext
        await supabase.storage.from('documentos-afiliados').upload(
          `${result.id}/${tipo}_${Date.now()}.${finalExt}`, fileToUp, { upsert: false }
        )
      }
      } else {
  ...
  const padresEnHijos = (form.hijos || []).filter(h => h.tipo === 'padre' && h.nombre?.trim()).length
  const cuota = calcularCuota(form.tipo_inscripcion, padresEnHijos)
    ...
    <div>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'1rem',marginBottom:'1rem',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'12px',alignItems:'end'}}>
          <div className="form-group">
            ...
            <select className="form-select" value={filterEstado} onChange={e => setFilter(e.target.value)}>
              ...
              <option value="en_carencia">En carencia</option>
              <option value="pendiente_pago">Pendiente pago</option>
              <option value="baja_voluntaria">Baja voluntaria</option>
              <option value="baja_impago">Baja por impago</option>
            </select>
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={filterProvincia} onChange={e => setFilterProv(e.target.value)}>
              <option value="todos">Todas</option>
              {['A Coruña','Pontevedra','Lugo','Ourense'].map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
      ...
      {filtrosActivos > 0 && (
        <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'0.75rem'}}>
          ...
          {filterProvincia !== 'todos' && <span style={{fontSize:'11px',padding:'3px 10px',borderRadius:'20px',background:'var(--accent-bg)',color:'var(--accent-dk)',display:'flex',alignItems:'center',gap:'5px'}}>Provincia: {filterProvincia} <button onClick={() => setFilterProv('todos')} style={{background:'none',border:'none',cursor:'pointer',color:'var(--accent-dk)',lineHeight:1,padding:0,fontSize:'13px'}}>×</button></span>}
        </div>
      )}
                  ...
                  {filtered.length === 0
                    ...
                    : filtered.map(a => (
                      <tr key={a.id}>
                        ...
                        <td><EstadoBadge estado={a.estado} estadoCalculado={a.estado_calculado} /></td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal" style={{maxWidth:'580px'}}>
            ...
            <div className="modal-header">
              <div>
                ...
                <div style={{fontSize:'11px',color:'var(--muted)',marginTop:'2px'}}>
                  Paso {paso} de 4 · {['Datos personales','Contacto y ubicación','Inscripción','Documentos'][paso-1]}
                </div>
              </div>
            ...
            <div style={{height:'3px',background:'var(--border)',position:'relative'}}>
              <div style={{height:'100%',background:'var(--accent)',borderRadius:'2px',transition:'width 0.3s ease',width:`${(paso/4)*100}%`}}/>
            </div>

            {/* Step indicators */}
            <div style={{display:'flex',padding:'1rem 1.5rem 0',gap:'0'}}>
              {[['1','Datos personales'],['2','Contacto'],['3','Inscripción'],['4','Documentos']].map(([n, label], i) => {
                const num = i + 1
                ...
                return (
                  <div key={n} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'4px',position:'relative'}}>
                    ...
                    {i < 3 && <div style={{position:'absolute',top:'12px',left:'50%',right:0,height:'1px',background: num < paso ? 'var(--accent)' : 'var(--border)'}}/>}
                    <div style={{
            ...
            <div className="modal-body" style={{paddingTop:'1.25rem'}}>
                  ...
                  <div className="form-grid" style={{marginBottom:'1rem'}}>
                    ...
                    <div className="form-group" style={{position:'relative'}}>
                      ...
                      <input className="form-input" value={form.pais_origen}
                        ...
                        onChange={e => {
                          updateForm('pais_origen', e.target.value)
                          setPaisFijado(false)
                          if(errores.pais_origen) setErrores(er=>({...er,pais_origen:null}))
                        }}
                        placeholder="Escribe para buscar país..." autoComplete="off" />
                      {/* Dropdown */}
                      {form.pais_origen && !paisFijado && (() => {
                        const filtrados = PAISES.filter(p => p.toLowerCase().includes(form.pais_origen.toLowerCase())).slice(0, 8)
                        return filtrados.length > 0 ? (
                          <div style={{
                            ...
                            {filtrados.map(p => (
                              <div key={p}
                                style={{padding:'8px 12px',cursor:'pointer',fontSize:'13px',borderBottom:'0.5px solid var(--border)'}}
                                onMouseEnter={e => e.currentTarget.style.background='var(--bg)'}
                                onMouseLeave={e => e.currentTarget.style.background='transparent'}
                                onClick={() => { updateForm('pais_origen', p); setPaisFijado(true); if(errores.pais_origen) setErrores(er=>({...er,pais_origen:null})) }}>
                                {p}
                              </div>
                            ))}
                          </div>
              ...
              {paso === 3 && (
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'1rem'}}>
                    {[
                      {val:'familia', titulo:'Familia', desc:'Matrimonio, o padre/madre con hijos menores a cargo', precio:'50€/año'},
                  ...
                  {(form.tipo_inscripcion === 'familia' || Number(form.padres_convivientes) > 0) && (
                    <div style={{marginBottom:'1rem'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                        <label className="form-label" style={{margin:0}}>
                          {form.tipo_inscripcion === 'familia'
                            ? form.estado_civil === 'Casado/a' ? 'Miembros de la unidad familiar' : 'Hijos menores'
                            : 'Padres convivientes'}
                        </label>
                        <div style={{display:'flex',gap:'6px',flexWrap:'wrap',justifyContent:'flex-end'}}>
                          {form.tipo_inscripcion === 'familia' && form.estado_civil === 'Casado/a' && (
                        ...
                        ? <div style={{padding:'10px 14px',background:'var(--bg)',borderRadius:'var(--radius-sm)',fontSize:'12px',color:'var(--muted)',textAlign:'center'}}>
                            {form.tipo_inscripcion === 'familia'
                              ? form.estado_civil === 'Casado/a'
                                ? 'Añade el cónyuge (obligatorio) y los hijos menores si los hay.'
                                : 'Añade los hijos menores cubiertos por esta inscripción.'
                              : 'Usa el botón + Padre/Madre para añadir los padres convivientes.'}
                          </div>
                      ...
                      {errores.padres_faltantes && (
                        <div className="alert alert-error" style={{marginTop:'8px',marginBottom:0,fontSize:'12px'}}>
                          {errores.padres_faltantes}
                        </div>
                      )}
                      {errores.conyuge_faltante && (
                        <div className="alert alert-error" style={{marginTop:'8px',marginBottom:0,fontSize:'12px'}}>
                          {errores.conyuge_faltante}
                        </div>
                      )}
                  ...
                  <div className="cuota-box" style={{marginBottom:'1rem'}}>
                    <span style={{fontSize:'13px',color:'var(--accent-dk)',fontWeight:500}}>Cuota anual calculada</span>
                  ...
                  {modal === 'nuevo' && (
                    <div style={{
                      border: errores.aceptaReglamento ? '1px solid var(--danger-tx)' : '1px solid var(--border)',
                      borderRadius:'var(--radius)',padding:'14px',marginBottom:'1rem',
                      background: form.aceptaReglamento ? 'var(--accent-bg)' : 'var(--surface)'
                    }}>
                      <label style={{display:'flex',alignItems:'flex-start',gap:'10px',cursor:'pointer'}}>
                        <input type="checkbox" checked={form.aceptaReglamento}
                          onChange={e => { updateForm('aceptaReglamento', e.target.checked); if(e.target.checked) setErrores(er=>({...er,aceptaReglamento:null})) }}
                          style={{marginTop:'3px',flexShrink:0}} />
                        <div>
                          <div style={{fontSize:'13px',fontWeight:500,color: form.aceptaReglamento ? 'var(--accent-dk)' : 'var(--text)'}}>
                            Acepto el reglamento del fondo <span style={{color:'var(--danger-tx)'}}>*</span>
                          </div>
                          <div style={{fontSize:'11px',color:'var(--muted)',marginTop:'3px',lineHeight:1.5}}>
                            Art. 16 · Al marcar esta casilla declaro haber leído y aceptado el{' '}
                            <a href="/reglamento" target="_blank" style={{color:'var(--accent)'}}>reglamento del Fondo de Decesos y Repatriación – UCIDGAL</a>.
                            Esta acción constituye la validación formal del contrato entre ambas partes.
                          </div>
                        </div>
                      </label>
                      {errores.aceptaReglamento && (
                        <div style={{fontSize:'11px',color:'var(--danger-tx)',marginTop:'8px'}}>
                          {errores.aceptaReglamento}
                        </div>
                      )}
                    </div>
              ...
              {paso === 4 && (
                <div>
                  {[
                    { tipo: 'dni', label: 'DNI / NIE / Pasaporte', desc: 'Fotocopia del documento de identidad', required: true },
                    { tipo: 'empadronamiento', label: 'Empadronamiento colectivo', desc: 'Certificado de empadronamiento colectivo', required: true },
                    ...(form.matrimonio_mixto ? [{ tipo: 'acta_notarial', label: 'Acta notarial (Art. 15)', desc: 'Acta notarial del cónyuge indicando su voluntad de sepultura', required: false }] : []),
                  ].map(({ tipo, label, desc, required }) => {
                    const fileKey = `_doc_${tipo}`
                    const file = form[fileKey]
                    return (
                      <div key={tipo} style={{marginBottom:'1rem',padding:'14px',border:`1px solid ${file ? 'var(--accent)' : 'var(--border)'}`,borderRadius:'var(--radius)',background: file ? 'var(--accent-bg)' : 'var(--bg)'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                          <div>
                            <div style={{fontSize:'13px',fontWeight:500,color:'var(--text)'}}>
                              {label}
                              {required && <span style={{color:'var(--danger-tx)',marginLeft:'4px'}}>*</span>}
                              {!required && <span style={{fontSize:'10px',color:'var(--muted)',marginLeft:'6px',padding:'1px 6px',borderRadius:'20px',border:'1px solid var(--border)'}}>Opcional</span>}
                            </div>
                          ...
                          <label style={{cursor:'pointer',flexShrink:0}}>
                            <input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" style={{display:'none'}}
                              onChange={e => { if(e.target.files[0]) updateForm(fileKey, e.target.files[0]) }} />
                            <span className="btn btn-sm" style={{pointerEvents:'none',color:'var(--accent)',borderColor:'var(--accent)'}}>
                              ↑ Subir
                            </span>
                          </label>
                        </div>
                        {file
                          ? <div style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'12px',color:'var(--accent-dk)'}}>
                              <svg style={{width:'13px',height:'13px'}} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>
                              {file.name}
                              <button type="button" onClick={() => updateForm(fileKey, null)}
                                style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:'13px',marginLeft:'auto'}}>✕</button>
                            </div>
                          : <div style={{fontSize:'11px',color:'var(--muted)'}}>PDF, JPG o PNG · máx. 20MB · las imágenes se comprimen automáticamente</div>
                        }
                      </div>
                    )
                  })}
                  <div style={{background:'var(--bg)',borderRadius:'var(--radius-sm)',padding:'10px 14px',fontSize:'12px',color:'var(--muted)'}}>
                    Los documentos se guardan de forma segura. Podrás añadir o actualizar documentos desde la ficha del afiliado en cualquier momento.
                  </div>
                </div>
              )}
            ...
            <div className="modal-footer" style={{justifyContent:'space-between'}}>
              ...
              <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
                <span style={{fontSize:'11px',color:'var(--muted)'}}>{paso}/4</span>
                {paso < 4
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal" style={{maxWidth:'620px'}}>
            ...
            <div style={{display:'flex',borderBottom:'1px solid var(--border)',padding:'0 1.5rem'}}>
              {[['datos','Datos'],['documentos','Documentos'],['historial','Historial']].map(([tab, label]) => (
                <button key={tab} onClick={() => { setTabDetalle(tab); if (tab === 'historial' && historial.length === 0) fetchHistorial(selected.id); if (tab === 'documentos' && documentos.length === 0) fetchDocumentos(selected.id) }}
            ...
            <div className="modal-body">
              ...
              {tabDetalle === 'documentos' && (
                <div>
                  {[
                    { tipo: 'dni', label: 'DNI / NIE / Pasaporte', desc: 'Fotocopia del documento de identidad (PDF, JPG, PNG · máx. 5MB)' },
                    { tipo: 'empadronamiento', label: 'Empadronamiento colectivo', desc: 'Certificado de empadronamiento colectivo (PDF, JPG, PNG · máx. 5MB)' },
                    ...(selected.matrimonio_mixto ? [{ tipo: 'acta_notarial', label: 'Acta notarial (Art. 15)', desc: 'Acta notarial del cónyuge indicando su voluntad de sepultura (PDF, JPG, PNG · máx. 5MB)' }] : []),
                  ].map(({ tipo, label, desc }) => {
                    const docs = documentos.filter(d => d.name.startsWith(tipo))
                    return (
                      <div key={tipo} style={{marginBottom:'1.25rem',padding:'14px',border:'1px solid var(--border)',borderRadius:'var(--radius)',background:'var(--bg)'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                          <div>
                        ...
                        {loadingDocs
                          ...
                          : docs.length === 0
                            ? <div style={{fontSize:'12px',color:'var(--muted)',padding:'8px 0'}}>No hay documentos subidos todavía.</div>
                            : docs.map(doc => {
                                const path = `${selected.id}/${doc.name}`
                                const ext = doc.name.split('.').pop().toUpperCase()
                                const fecha = doc.created_at ? new Date(doc.created_at).toLocaleDateString('es-ES') : ''
                                return (
                                  <div key={doc.name} style={{display:'flex',alignItems:'center',gap:'10px',padding:'7px 10px',background:'var(--surface)',borderRadius:'var(--radius-sm)',border:'0.5px solid var(--border)',marginBottom:'6px'}}>
                                    <div style={{width:'32px',height:'32px',borderRadius:'6px',background:'var(--accent-bg)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:600,color:'var(--accent-dk)',flexShrink:0}}>{ext}</div>
                                    <div style={{flex:1,minWidth:0}}>
                                      <div style={{fontSize:'12px',fontWeight:500,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{doc.name}</div>
                                      {fecha && <div style={{fontSize:'11px',color:'var(--muted)'}}>{fecha}</div>}
                                    </div>
                                    <button className="btn btn-sm" onClick={() => verDocumento(path)}>Ver</button>
                                    <button className="btn btn-sm" style={{color:'var(--danger-tx)',borderColor:'rgba(163,45,45,0.25)'}}
                                      onClick={() => setConfirmar({
                                        titulo: 'Eliminar documento',
                                        mensaje: '¿Confirmas que deseas eliminar este documento? Esta acción no se puede deshacer.',
                                        labelConfirmar: 'Eliminar',
                                        tipo: 'danger',
                                        accion: () => eliminarDocumento(path)
                                      })}>✕</button>
                                  </div>
                                )
                              })
            ...
            <div className="modal-footer">
              <button className="btn btn-danger" onClick={() => setConfirmar({
                ...
                tipo: 'danger',
                accion: () => { cambiarEstado(selected.id, selected.estado, 'baja_voluntaria'); closeModal() }
              })}>Baja voluntaria</button>
