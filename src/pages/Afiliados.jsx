...
export default function Afiliados() {
  ...
  async function guardar() {
    ...
    if (error) {
      ...
      // Upload acta notarial if provided
      if (form._actaNotarial && form.matrimonio_mixto) {
        const ext = form._actaNotarial.name.split('.').pop().toLowerCase()
        const fileToUp = ['jpg','jpeg','png','webp'].includes(ext) ? await comprimirImagen(form._actaNotarial) : form._actaNotarial
        const finalExt = ['jpg','jpeg','png','webp'].includes(ext) ? 'jpg' : ext
        await supabase.storage.from('documentos-afiliados').upload(
          `${result.id}/acta_notarial_${Date.now()}.${finalExt}`, fileToUp, { upsert: false }
        )
      }
      } else {
                    ...
                    <div style={{border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'12px 14px',marginBottom:'1rem'}}>
                      ...
                      {form.matrimonio_mixto && (
                        <div style={{marginTop:'10px',paddingTop:'10px',borderTop:'1px solid var(--border)'}}>
                          <div style={{fontSize:'11px',fontWeight:500,color:'var(--muted)',marginBottom:'6px'}}>Acta notarial (opcional) — podrás subirla también desde la ficha del afiliado</div>
                          <label style={{cursor:'pointer',display:'inline-block'}}>
                            <input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" style={{display:'none'}}
                              onChange={e => updateForm('_actaNotarial', e.target.files[0])} />
                            <div style={{
                              display:'flex',alignItems:'center',gap:'8px',padding:'6px 12px',
                              border:'1px dashed var(--border-md)',borderRadius:'var(--radius-sm)',
                              fontSize:'12px',color:'var(--muted)',cursor:'pointer',
                              background: form._actaNotarial ? 'var(--accent-bg)' : 'transparent'
                            }}>
                              <svg style={{width:'13px',height:'13px',flexShrink:0}} viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM6.293 6.707a1 1 0 010-1.414l3-3a1 1 0 011.414 0l3 3a1 1 0 01-1.414 1.414L11 5.414V13a1 1 0 11-2 0V5.414L7.707 6.707a1 1 0 01-1.414 0z" clipRule="evenodd"/>
                              </svg>
                              {form._actaNotarial ? form._actaNotarial.name : 'Subir acta notarial (PDF, JPG, PNG)'}
                            </div>
                          </label>
                          {form._actaNotarial && (
                            <button type="button" onClick={() => updateForm('_actaNotarial', null)}
                              style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:'13px',marginLeft:'8px'}}>✕ Quitar</button>
                          )}
                        </div>
                      )}
                    </div>
