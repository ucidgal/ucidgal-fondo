...
function TarjetaVirtual({ afiliado, fechaPago, onClose }) {
  ...
  function imprimir() {
    ...
    w.document.write(`
      ...
      <style>
        body { margin:0; font-family: 'DM Sans', Arial, sans-serif; background:#f4f9fb; display:flex; align-items:center; justify-content:center; min-height:100vh; }
        ...
        .org { font-size:13px; font-weight:500; opacity:0.9; }
        .org-sub { font-size:11px; opacity:0.7; margin-top:1px; }
        .name { font-size:20px; font-weight:600; margin-bottom:4px; }
        .num { font-size:12px; opacity:0.8; letter-spacing:0.05em; }
        ...
        .carencia { background:#FAEEDA; color:#633806; }
      </style></head>
      <body><div class="card">
        <div class="header">
          <div class="header-top">
            <div class="logo">☽</div>
            <div><div class="org">Fondo Islámico UCIDGAL</div><div class="org-sub">Galicia · España</div></div>
          </div>
        ...
        <div class="body">
          ...
          <div class="row"><span class="lbl">Estado</span><span class="val">
            <span class="status ${carencia && carencia.activa ? 'carencia' : 'activo'}">
              ${carencia ? (carencia.activa ? 'En carencia' : 'Cobertura activa') : 'Pendiente pago'}
            </span>
          </span></div>
        ...
        <div class="footer">Fondo para el Entierro y Repatriación · UCIDGAL<br/>contacto@ucidgal.org</div>
      </div></body></html>
    `)
    w.document.close()
        ...
        <div className="modal-body">
          <div style={{background:'var(--accent)',borderRadius:'var(--radius-lg)',padding:'20px',color:'#fff',marginBottom:'1rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'12px'}}>
              ...
              <div>
                <div style={{fontSize:'12px',opacity:0.9,fontWeight:500}}>Fondo Islámico UCIDGAL</div>
                <div style={{fontSize:'11px',opacity:0.7}}>Galicia · España</div>
              </div>
            </div>
            <div style={{fontSize:'18px',fontWeight:500,marginBottom:'4px'}}>{afiliado.nombre} {afiliado.apellidos}</div>
            <div style={{fontSize:'12px',opacity:0.8}}>Nº {afiliado.num_inscripcion} · {new Date().getFullYear()}</div>
          </div>
          ...
          {[
            ...
            ['Estado cobertura', carencia
              ? <span className={`badge ${carencia.activa ? 'badge-pendiente' : 'badge-activo'}`}>{carencia.activa ? 'En carencia' : 'Cobertura activa'}</span>
              : <span className="badge badge-pendiente">Pendiente pago</span>
            ],
            carencia ? ['Detalle carencia', <span style={{fontSize:'12px',color:'var(--muted)'}}>{carencia.texto}</span>] : null,
          ...
          <p style={{fontSize:'11px',color:'var(--muted)',marginTop:'1rem',padding:'8px',background:'var(--bg)',borderRadius:'var(--radius-sm)'}}>
            Tarjeta gratuita · Art. 9 y 10 del Reglamento UCIDGAL. Válida hasta el último día de febrero de {new Date().getFullYear() + 1}.
          </p>
        </div>
