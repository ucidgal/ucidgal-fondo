...
export default function Afiliados() {
  ...
  const cuota = calcularCuota(form.tipo_inscripcion, (form.hijos||[]).filter(h=>h.tipo==='padre'&&h.nombre?.trim()).length)
                    ...
                    <div style={{marginBottom:'1rem'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                        ...
                        <div style={{display:'flex',gap:'6px',flexWrap:'wrap',justifyContent:'flex-end'}}>
                          ...
                          {(form.hijos||[]).filter(h=>h.tipo==='padre').length < 2 && (
...
{form.tipo_inscripcion === 'familia' ? 'Añade el cónyuge e hijos menores cubiertos por esta inscripción.' : 'Usa el botón + Padre/Madre para añadir los padres convivientes.'}
                      ...
                      {form.tipo_inscripcion === 'familia' && form.hijos?.some(h => h.tipo === 'hijo' && h.fecha_nacimiento && Math.floor((new Date()-new Date(h.fecha_nacimiento))/(365.25*86400000)) >= 18) && (
  ...
  {form.tipo_inscripcion === 'familia' ? `50€ base · ${1 + (form.hijos?.filter(h=>h.nombre?.trim()).length||0)} miembros` : '25€ base'}
                        {(form.hijos||[]).filter(h=>h.tipo==='padre'&&h.nombre?.trim()).length > 0
                          ? ` + ${(form.hijos||[]).filter(h=>h.tipo==='padre'&&h.nombre?.trim()).length * 25}€ (${(form.hijos||[]).filter(h=>h.tipo==='padre'&&h.nombre?.trim()).length} padre(s))`
                          : ''}
