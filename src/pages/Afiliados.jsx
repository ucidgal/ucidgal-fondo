...
export default function Afiliados() {
  ...
  async function guardar() {
    ...
    // Exclude virtual fields from the view and file upload fields
    const CAMPOS_EXCLUIDOS = new Set(['estado_calculado','ultima_fecha_pago'])
    const cleanFormData = Object.fromEntries(
      Object.entries(formData).filter(([k]) => !k.startsWith('_doc_') && !CAMPOS_EXCLUIDOS.has(k))
    )
    const payload = {
      ...cleanFormData,
    ...
    if (error) {
      ...
      if (modal === 'nuevo') {
        await registrarHistorial(result.id, 'creacion', [{ campo: 'registro', anterior: null, nuevo: form.aceptaReglamento ? `Alta en el sistema por ${email} · Reglamento aceptado (Art. 16)` : `Alta en el sistema por ${email}` }], email)
                          ...
                          <div style={{flex:1,minWidth:0,paddingBottom:'4px'}}>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px',marginBottom:'4px'}}>
                              <div style={{fontSize:'12px',fontWeight:500,color:'var(--text)'}}>
                                {h.accion === 'creacion'     && (h.valor_nuevo?.includes('por') ? h.valor_nuevo : `Alta en el sistema · Creado por: ${h.usuario_email}`)}
