...
export default function Afiliados() {
  ...
  function addHijo() {
    setForm(f => ({ ...f, hijos: [...(f.hijos || []), { nombre: '', fecha_nacimiento: '' }] }))
  }
                ...
                <div>
                  ...
                  <div className="form-grid-3" style={{marginBottom:'1rem'}}>
                    <div className="form-group">
                      ...
                      <input className="form-input" type="number" min="1" max="20" value={form.num_miembros}
                        onChange={e => updateForm('num_miembros', e.target.value)} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Padres convivientes</label>
                      <input className="form-input" type="number" min="0" max="2" value={form.padres_convivientes}
                  ...
                  {form.tipo_inscripcion === 'familia' && (
                    <div style={{marginBottom:'1rem'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                        <label className="form-label" style={{margin:0}}>Hijos menores de edad cubiertos</label>
                        <button className="btn btn-sm" onClick={addHijo} type="button"
                      ...
                      {(!form.hijos || form.hijos.length === 0)
                        ? <div style={{padding:'10px 14px',background:'var(--bg)',borderRadius:'var(--radius-sm)',fontSize:'12px',color:'var(--muted)',textAlign:'center'}}>
                            No hay hijos registrados. Haz clic en "Añadir hijo" para incluirlos.
                          </div>
                        : <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                            {form.hijos.map((h, i) => (
                              <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 1fr auto',gap:'8px',alignItems:'flex-start',padding:'10px 12px',background:'var(--bg)',borderRadius:'var(--radius-sm)',border:'0.5px solid var(--border)'}}>
                                ...
                                <div className="form-group" style={{margin:0}}>
                                  ...
                                  <input className="form-input" type="date" value={h.fecha_nacimiento}
                                    onChange={e => updateHijo(i, 'fecha_nacimiento', e.target.value)} />
                                  {h.fecha_nacimiento && (
                                    <span style={{fontSize:'10px',color:'var(--muted)',marginTop:'2px',display:'block'}}>
                                      ...
                                      {Math.floor((new Date() - new Date(h.fecha_nacimiento)) / (365.25*86400000)) >= 18
                                        ? <span style={{color:'var(--danger-tx)'}}> · Mayor de edad — no cubierto</span>
                                        : ''}
                                    </span>
                                  )}
                                </div>
                                <button onClick={() => removeHijo(i)} type="button"
                                  style={{marginTop:'18px',background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:'16px',padding:'4px'}}>
                                  ✕
                                </button>
                              </div>
                            ))}
                      ...
                      {form.hijos?.some(h => h.fecha_nacimiento && Math.floor((new Date() - new Date(h.fecha_nacimiento)) / (365.25*86400000)) >= 18) && (
                  ...
                  <div className="cuota-box" style={{marginBottom:'1rem'}}>
                    <div>
                      ...
                      <div style={{fontSize:'11px',color:'var(--accent-dk)',opacity:0.8,marginTop:'2px'}}>
                        {form.tipo_inscripcion === 'familia' ? '50€ base' : '25€ base'}
                  ...
                  <div style={{background:'var(--bg)',borderRadius:'var(--radius)',padding:'12px 14px',fontSize:'12px'}}>
                    ...
                    {[
                      ...
                      ['Cuota anual', `${cuota}€`],
                      ['Hijos registrados', form.hijos?.filter(h=>h.nombre).length > 0 ? form.hijos.filter(h=>h.nombre).map(h=>h.nombre).join(', ') : 'Ninguno'],
                    ].map(([l,v]) => (
          ...
          <div className="modal" style={{maxWidth:'620px'}}>
            ...
            <div className="modal-body">
              {tabDetalle === 'datos' && (
                ...
                ['Hijos cubiertos', selected.hijos?.length > 0
                  ? <div style={{display:'flex',flexDirection:'column',gap:'3px'}}>
                      {selected.hijos.map((h,i) => (
                        <span key={i} style={{fontSize:'12px'}}>
                          {h.nombre || '(sin nombre)'}
                          {h.fecha_nacimiento ? ` · ${new Date(h.fecha_nacimiento).toLocaleDateString('es-ES')} · ${Math.floor((new Date()-new Date(h.fecha_nacimiento))/(365.25*86400000))} años` : ''}
                        </span>
                      ))}
                    </div>
                  : 'Ninguno'
