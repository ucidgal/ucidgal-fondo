...
const EMPTY_FORM = {
  ...
  tipo_inscripcion: 'familia', padres_convivientes: 0,
...
export default function Afiliados() {
  ...
  async function guardar() {
    ...
    const payload = {
      ...
      num_miembros: form.tipo_inscripcion === 'familia' ? 1 + (form.hijos?.filter(h => h.nombre?.trim()).length || 0) : 1,
                  ...
                  {form.tipo_inscripcion === 'familia' && (
                    <div style={{marginBottom:'1rem'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                        <label className="form-label" style={{margin:0}}>Miembros de la unidad familiar</label>
                      ...
                      {(!form.hijos || form.hijos.length === 0)
                        ? <div style={{padding:'10px 14px',background:'var(--bg)',borderRadius:'var(--radius-sm)',fontSize:'12px',color:'var(--muted)',textAlign:'center'}}>
                            Añade el cónyuge e hijos menores cubiertos por esta inscripción.
                          </div>
                                ...
                                <div key={i} style={{padding:'10px 12px',background:'var(--bg)',borderRadius:'var(--radius-sm)',border:`0.5px solid ${esConyuge ? 'var(--accent)' : 'var(--border)'}`}}>
                                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                                    <span style={{fontSize:'11px',fontWeight:500,padding:'2px 8px',borderRadius:'20px',
                                      background: esConyuge ? 'var(--accent-bg)' : 'var(--bg)',
                                      color: esConyuge ? 'var(--accent-dk)' : 'var(--muted)',
                                      border:`0.5px solid ${esConyuge ? 'var(--accent)' : 'var(--border)'}`}}>
                                      {esConyuge ? 'Cónyuge' : 'Hijo/a'}
                                    </span>
                                  ...
                                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                                    <div className="form-group" style={{margin:0}}>
                                      ...
                                      <input className="form-input" value={h.nombre || ''}
                                        onChange={e => updateHijo(i, 'nombre', e.target.value)} placeholder="Nombre" />
                                    </div>
                                    <div className="form-group" style={{margin:0}}>
                                      <label className="form-label">Apellidos</label>
                                      <input className="form-input" value={h.apellidos || ''}
                                        onChange={e => updateHijo(i, 'apellidos', e.target.value)} placeholder="Apellidos" />
                                    </div>
                                    <div className="form-group" style={{margin:0}}>
                                      <label className="form-label">DNI / NIE</label>
                                      <input className="form-input" value={h.dni_nie || ''}
                                        onChange={e => updateHijo(i, 'dni_nie', e.target.value.toUpperCase())} placeholder="X0000000X" />
                                    </div>
                                    <div className="form-group" style={{margin:0}}>
                                      ...
                                      {edad !== null && (
                                        <span style={{fontSize:'10px',color: (!esConyuge && edad >= 18) ? 'var(--danger-tx)' : 'var(--muted)',marginTop:'2px',display:'block'}}>
                                          {edad} años{!esConyuge && edad >= 18 ? ' · Mayor de edad — no cubierto' : ''}
                                        </span>
                                      )}
                                    </div>
                                  </div>
                      ...
                      {form.hijos?.some(h => h.tipo !== 'conyuge' && h.fecha_nacimiento && Math.floor((new Date()-new Date(h.fecha_nacimiento))/(365.25*86400000)) >= 18) && (
                  ...
                  <div className="cuota-box" style={{marginBottom:'1rem'}}>
                    <div>
                      ...
                      <div style={{fontSize:'11px',color:'var(--accent-dk)',opacity:0.8,marginTop:'2px'}}>
                        {form.tipo_inscripcion === 'familia' ? `50€ base · ${2 + (form.hijos?.filter(h=>h.nombre).length||0)} miembros` : '25€ base'}
                  ...
                  <div style={{background:'var(--bg)',borderRadius:'var(--radius)',padding:'12px 14px',fontSize:'12px'}}>
                    ...
                    {[
                      ...
                      form.hijos?.filter(h=>h.tipo==='conyuge'&&h.nombre).length > 0
                        ...
                        : null,
                      form.hijos?.filter(h=>h.tipo!=='conyuge'&&h.nombre).length > 0
                        ? ['Hijos', form.hijos.filter(h=>h.tipo!=='conyuge'&&h.nombre).map(h=>h.nombre).join(', ')]
                        : ['Hijos', 'Ninguno'],
                    ].filter(Boolean).map(([l,v]) => (
                      ...
                      {selected.hijos.map((h,i) => (
                        <div key={i} style={{fontSize:'12px',display:'flex',alignItems:'center',gap:'6px'}}>
                          <span style={{fontSize:'10px',padding:'1px 6px',borderRadius:'20px',
                            background: h.tipo==='conyuge' ? 'var(--accent-bg)' : 'var(--bg)',
                            color: h.tipo==='conyuge' ? 'var(--accent-dk)' : 'var(--muted)',
                            border:'0.5px solid var(--border)',flexShrink:0}}>
                            {h.tipo === 'conyuge' ? 'Cónyuge' : 'Hijo/a'}
                          </span>
