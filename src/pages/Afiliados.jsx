  ...
  {form.tipo_inscripcion === 'familia' ? `50€ base · ${2 + (form.hijos?.filter(h=>h.nombre?.trim()).length||0)} miembros` : '25€ base'}
                  ...
                  <div style={{background:'var(--bg)',borderRadius:'var(--radius)',padding:'12px 14px',fontSize:'12px'}}>
                    ...
                    {[
                      ...
                      ['Total miembros', (() => {
                        const conyuges = (form.hijos||[]).filter(h=>h.tipo==='conyuge'&&h.nombre?.trim()).length
                        const hijos_ = (form.hijos||[]).filter(h=>h.tipo==='hijo'&&h.nombre?.trim()).length
                        const padres_ = (form.hijos||[]).filter(h=>h.tipo==='padre'&&h.nombre?.trim()).length
                        const total = 1 + conyuges + hijos_ + padres_
                        const partes = [`1 titular`]
                        if (conyuges > 0) partes.push(`${conyuges} cónyuge`)
                        if (hijos_ > 0) partes.push(`${hijos_} hijo(s)`)
                        if (padres_ > 0) partes.push(`${padres_} padre(s)`)
