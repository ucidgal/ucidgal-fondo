...
export default function Siniestros() {
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: s }, { data: a }] = await Promise.all([
      ...
      supabase.from('afiliados').select('id, nombre, apellidos, num_inscripcion, estado, matrimonio_mixto, created_at').order('num_inscripcion')
    ])
    setSiniestros(s || [])
    setAfiliados(a || [])
    setLoading(false)
  ...
  const afiliadoInactivo = afiliadoSelec && afiliadoSelec.estado !== 'activo'
                    ...
                    <select className="form-select" value={form.afiliado_id} onChange={e => updateForm('afiliado_id', e.target.value)}
                      ...
                      {afiliados.map(a => (
                        <option key={a.id} value={a.id}>
                          {a.num_inscripcion} · {a.nombre} {a.apellidos}{a.estado !== 'activo' ? ` (${a.estado})` : ''}
                        </option>
                      ))}
                    </select>
                    {afiliadoInactivo && (
