  ...
  function exportarExcel() {
    ...
    script.onload = () => {
      ...
      const datos = filtered.map(s => ({
        ...
        'Tipo':               labels[s.tipo_fallecido] || '—',
        'Titular':            `${s.afiliados?.nombre || ''} ${s.afiliados?.apellidos || ''}`.trim(),
        'Fecha':              s.fecha_fallecimiento || '—',
        'Destino':            s.pais_destino || s.lugar_fallecimiento || '—',
        'Gasto':              s.gasto_total ? Number(s.gasto_total).toLocaleString('es-ES') + '€' : '—',
        'Estado':             labels[s.estado],
      }))
      const ws = XLSX.utils.json_to_sheet(datos)
      ws['!cols'] = [{wch:16},{wch:22},{wch:14},{wch:28},{wch:14},{wch:20},{wch:14},{wch:12}]
  ...
  function exportarPDF() {
    ...
    const filas = filtered.map(s => `
      <tr>
        ...
        <td style="color:#6b6b68;font-size:11px">${labels[s.tipo_fallecido] || '—'}</td>
        <td>${s.afiliados?.nombre || ''} ${s.afiliados?.apellidos || ''}</td>
        <td>${s.fecha_fallecimiento || '—'}</td>
        <td>${s.pais_destino || s.lugar_fallecimiento || '—'}</td>
        <td>${s.gasto_total ? Number(s.gasto_total).toLocaleString('es-ES') + '€' : '—'}</td>
