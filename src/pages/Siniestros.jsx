...
export default function Siniestros() {
  ...
  const [busqFallecido, setBusqFallecido]   = useState('')
  const [fallecidoFijado, setFallecidoFijado] = useState(false)
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: s }, { data: a }] = await Promise.all([
      ...
      supabase.from('afiliados_estado_real').select('id, nombre, apellidos, num_inscripcion, estado, estado_calculado, matrimonio_mixto, created_at, hijos').order('num_inscripcion')
    ])
    setSiniestros(s || [])
    setAfiliados(a || [])
    setLoading(false)
  ...
  useEffect(() => {
    ...
    if (filterAnio   !== 'todos') r = r.filter(s => s.fecha_fallecimiento && new Date(s.fecha_fallecimiento).getFullYear() === Number(filterAnio))
    setFiltered(r)
  }, [siniestros, filterEstado, filterAnio])
  ...
  function exportarExcel() {
    ...
    script.onload = () => {
      ...
      const datos = filtered.map(s => ({
        'Nº Expediente':      s.num_expediente,
        'Fallecido':          s.nombre_fallecido,
        'Tipo servicio':      s.tipo_servicio === 'repatriacion' ? 'Repatriación' : 'Entierro local',
        'Tipo fallecido':     labels[s.tipo_fallecido] || '—',
        'Afiliado titular':   `${s.afiliados?.nombre || ''} ${s.afiliados?.apellidos || ''}`.trim(),
        'Nº inscripción':     s.afiliados?.num_inscripcion || '—',
        'Fecha fallecimiento':s.fecha_fallecimiento || '—',
        'Destino / Lugar':    s.pais_destino || s.lugar_fallecimiento || '—',
        'Gasto total (€)':    s.gasto_total ? Number(s.gasto_total) : '',
        'Descripción gasto':  s.gasto_descripcion || '',
        'Cert. defunción':    s.cert_defuncion ? 'Sí' : 'No',
        'Permiso traslado':   s.permiso_traslado ? 'Sí' : 'No',
        'Estado':             labels[s.estado] || s.estado,
      }))
      const ws = XLSX.utils.json_to_sheet(datos)
      ws['!cols'] = [{wch:14},{wch:22},{wch:16},{wch:16},{wch:26},{wch:14},{wch:18},{wch:20},{wch:14},{wch:24},{wch:14},{wch:14},{wch:12}]
  ...
  function exportarPDF() {
    ...
    const filas = filtered.map(s => `
      <tr>
        <td class="mono">${s.num_expediente}</td>
        <td style="font-weight:500">${s.nombre_fallecido}</td>
        <td style="color:#6b6b68;font-size:10px">${s.tipo_servicio === 'repatriacion' ? 'Repatriación' : 'Entierro local'}</td>
        <td style="color:#6b6b68;font-size:10px">${labels[s.tipo_fallecido] || '—'}</td>
        <td>${s.afiliados?.nombre || ''} ${s.afiliados?.apellidos || ''}</td>
        <td>${s.fecha_fallecimiento || '—'}</td>
        <td>${s.pais_destino || s.lugar_fallecimiento || '—'}</td>
        <td style="font-weight:500">${s.gasto_total ? Number(s.gasto_total).toLocaleString('es-ES') + '€' : '—'}</td>
        <td><span class="estado-${s.estado}">${labels[s.estado] || s.estado}</span></td>
      </tr>`).join('')
    ...
    w.document.write(`
      ...
      <style>
        ...
        th { background: #29BAD4; color: #fff; text-align: left; padding: 7px 8px; font-size: 9px; text-transform: uppercase; letter-spacing:0.04em; }
      ...
      <body>
        ...
        <table>
          <thead><tr><th>Expediente</th><th>Fallecido</th><th>Servicio</th><th>Tipo</th><th>Titular</th><th>Fecha</th><th>Destino/Lugar</th><th>Gasto</th><th>Estado</th></tr></thead>
          <tbody>${filas}</tbody>
        </table>
  ...
  function openNuevo()    { setForm(EMPTY_FORM); setMsg(null); setBusqFallecido(''); setFallecidoFijado(false); setModal('nuevo') }
  function openDetalle(s) { setSelected(s); setModal('detalle') }
  function openEditar(s)  {
    setSelected(s)
    setForm({
      nombre_fallecido:    s.nombre_fallecido,
      afiliado_id:         s.afiliado_id,
      fecha_fallecimiento: s.fecha_fallecimiento || '',
      lugar_fallecimiento: s.lugar_fallecimiento || '',
      tipo_fallecido:      s.tipo_fallecido || 'adulto',
      tipo_servicio:       s.tipo_servicio,
      pais_destino:        s.pais_destino || '',
      cert_defuncion:      s.cert_defuncion,
      permiso_traslado:    s.permiso_traslado,
      observaciones:       s.observaciones || '',
      estado:              s.estado,
      gasto_total:         s.gasto_total || '',
      gasto_descripcion:   s.gasto_descripcion || ''
    })
    setMsg(null)
    setModal('editar')
  }
  ...
  // Build searchable list of fallecidos: titular + all beneficiarios
  const candidatosFallecido = []
  afiliados.forEach(a => {
    candidatosFallecido.push({
      nombre: `${a.nombre} ${a.apellidos}`,
      tipo: 'Titular',
      afiliado_id: a.id,
      afiliado: `${a.num_inscripcion} · ${a.nombre} ${a.apellidos}`
    })
    ;(a.hijos || []).forEach(h => {
      if (h.nombre?.trim()) {
        candidatosFallecido.push({
          nombre: `${h.nombre} ${h.apellidos || ''}`.trim(),
          tipo: h.tipo === 'conyuge' ? 'Cónyuge' : h.tipo === 'padre' ? 'Padre/Madre' : 'Hijo/a',
          afiliado_id: a.id,
          afiliado: `${a.num_inscripcion} · ${a.nombre} ${a.apellidos}`
        })
      }
  ...
  const candidatosFiltrados = busqFallecido.trim().length >= 2
    ? candidatosFallecido.filter(c => c.nombre.toLowerCase().includes(busqFallecido.toLowerCase()))
    : []
  ...
  const afiliadoInactivo = afiliadoSelec && !['activo', 'en_carencia'].includes(afiliadoSelec.estado_calculado || afiliadoSelec.estado)
  ...
  async function guardar() {
    ...
    const finCobertura = new Date(finCarencia)
    finCobertura.setFullYear(finCobertura.getFullYear() + 1)

    if (fechaFallec > finCobertura) {
      setMsg({ type: 'error', text: `Art. 11: la cobertura de la póliza venció el ${finCobertura.toLocaleDateString('es-ES')}. El afiliado debe renovar la cuota para tener cobertura vigente.` })
      return
    }
    setSaving(true)
    ...
    const payload = {
      ...
      gasto_total: form.gasto_total ? Number(form.gasto_total) : null,
      updated_at: new Date().toISOString()
    }
    let error
    if (modal === 'editar') {
      const result = await supabase.from('siniestros').update(payload).eq('id', selected.id)
      error = result.error
    } else {
      const numExp = await genExpediente()
      const result = await supabase.from('siniestros').insert({ ...payload, num_expediente: numExp })
      error = result.error
    }
    ...
    <div>
      ...
      <div className="toolbar">
        <div className="toolbar-left">
          <select className="form-select" style={{width:'auto'}} value={filterAnio} onChange={e => setFilterAnio(e.target.value)}>
        ...
        {loading
          ...
          : <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {filtered.length === 0
                    ? <tr><td colSpan={9} style={{textAlign:'center',color:'var(--muted)',padding:'2rem'}}>No hay expedientes.</td></tr>
                    : filtered.map(s => (
                      <tr key={s.id}>
                        ...
                        <td style={{display:'flex',gap:'6px'}}>
                          <button className="btn btn-sm" onClick={() => openDetalle(s)}>Ver</button>
                          <button className="btn btn-sm" onClick={() => openEditar(s)}>Editar</button>
                        </td>
                      </tr>
                    ))
                  }
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">{modal === 'editar' ? `Editar expediente · ${selected?.num_expediente}` : 'Apertura de expediente'}</div>
            ...
            <div className="modal-body">
              ...
              <div className="form-grid" style={{marginBottom:'1rem'}}>
                <div className="form-group full" style={{position:'relative'}}>
                  ...
                  <input className="form-input" value={busqFallecido}
                    onChange={e => {
                      setBusqFallecido(e.target.value)
                      updateForm('nombre_fallecido', e.target.value)
                      updateForm('afiliado_id', '')
                      setFallecidoFijado(false)
                    }}
                    placeholder="Escribe para buscar entre afiliados y beneficiarios..." autoFocus />
                  {candidatosFiltrados.length > 0 && !fallecidoFijado && (
                    <div ref={dropdownRef} style={{
                      ...
                      {candidatosFiltrados.map((c, i) => (
                        <div key={i}
                          style={{padding:'9px 14px',cursor:'pointer',borderBottom:'0.5px solid var(--border)',display:'flex',alignItems:'center',gap:'10px'}}
                          onMouseEnter={e => e.currentTarget.style.background='var(--bg)'}
                          onMouseLeave={e => e.currentTarget.style.background='transparent'}
                          onClick={() => {
                            setBusqFallecido(c.nombre)
                            updateForm('nombre_fallecido', c.nombre)
                            updateForm('afiliado_id', c.afiliado_id)
                            setFallecidoFijado(true)
                          }}>
                          <div style={{flex:1}}>
                            <div style={{fontSize:'13px',fontWeight:500,color:'var(--text)'}}>{c.nombre}</div>
                            <div style={{fontSize:'11px',color:'var(--muted)'}}>{c.afiliado}</div>
                          </div>
                          <span style={{fontSize:'10px',padding:'2px 8px',borderRadius:'20px',flexShrink:0,
                            background: c.tipo==='Titular' ? 'var(--accent-bg)' : 'var(--bg)',
                            color: c.tipo==='Titular' ? 'var(--accent-dk)' : 'var(--muted)',
                  ...
                  {form.afiliado_id && (
                    <div style={{fontSize:'11px',color:'var(--accent-dk)',marginTop:'4px',display:'flex',alignItems:'center',gap:'4px'}}>
                      <svg style={{width:'11px',height:'11px'}} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>
                      Afiliado titular cargado automáticamente
                    </div>
                  )}
                </div>
                ...
                {!esAborto && (<>
                  <div className="form-group full">
                    ...
                    <div style={{padding:'8px 10px',background:'var(--bg)',border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',fontSize:'13px',
                      color: form.afiliado_id ? 'var(--text)' : 'var(--muted)'}}>
                      {form.afiliado_id && afiliadoSelec
                        ? `${afiliadoSelec.num_inscripcion} · ${afiliadoSelec.nombre} ${afiliadoSelec.apellidos}`
                        : 'Se cargará automáticamente al seleccionar el fallecido'}
                    </div>
                    {afiliadoInactivo && (
                      <div className="alert alert-error" style={{marginTop:'6px',marginBottom:0}}>
                        Art. 11 · Este afiliado está en estado "{afiliadoSelec?.estado_calculado || afiliadoSelec?.estado}" y no tiene derecho al servicio.
                      </div>
                    )}
                ...
                <div className="form-grid" style={{marginBottom:'1rem'}}>
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="number" min="0" step="0.01" value={form.gasto_total}
                      onChange={e => updateForm('gasto_total', Math.max(0, parseFloat(e.target.value) || 0))} placeholder="0.00" />
            ...
            <div className="modal-footer">
              ...
              {!esAborto && (
                <button className="btn btn-primary" onClick={guardar} disabled={saving}>{saving ? 'Guardando...' : modal === 'editar' ? 'Guardar cambios' : 'Abrir expediente'}</button>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              {[
                ...
                <div className="detail-row" key={idx}><span className="detail-label">{l}</span><span>{v}</span></div>
              ))}
              ...
              <div style={{marginTop:'1.25rem'}}>
                ...
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap',marginTop:'8px'}}>
                  <button className={`btn btn-sm ${selected.cert_defuncion === 'recibido' ? 'btn-primary' : ''}`}
                    onClick={() => {
                      const nuevo = selected.cert_defuncion === 'recibido' ? 'pendiente' : 'recibido'
                      actualizarCampo(selected.id, 'cert_defuncion', nuevo)
                      setSelected(s => ({...s, cert_defuncion: nuevo}))
                    }}>
                    {selected.cert_defuncion === 'recibido' ? '✓ Cert. defunción recibido' : 'Cert. defunción recibido'}
                  </button>
                  <button className={`btn btn-sm ${selected.permiso_traslado === 'obtenido' ? 'btn-primary' : ''}`}
                    onClick={() => {
                      const nuevo = selected.permiso_traslado === 'obtenido' ? 'pendiente' : 'obtenido'
                      actualizarCampo(selected.id, 'permiso_traslado', nuevo)
                      setSelected(s => ({...s, permiso_traslado: nuevo}))
                    }}>
                    {selected.permiso_traslado === 'obtenido' ? '✓ Permiso traslado obtenido' : 'Permiso traslado obtenido'}
                  </button>
                </div>
              </div>
            </div>
